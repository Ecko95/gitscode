...
describe("build_turn_start_command", () => {
	it("builds a valid thread.turn.start command with a review-comment body", () => {
		const cmd = build_turn_start_command("thread-123", {
			text: "fix this", filePath: "a.ts", startIndex: 1, endIndex: 2, diff: "-a\n+b",
		});
...
function start_mock_gits(handler: (url: string, method: string) => unknown): Promise<{ origin: string; server: Server }> {
...
describe("run_crit_agent", () => {
	...
	it("dispatches the turn and returns the assistant reply text", async () => {
		...
		mock = await start_mock_gits((url, method) => {
			if (url.endsWith("/api/orchestration/dispatch") && method === "POST") { dispatched = true; return {}; }
			if (url.endsWith("/api/orchestration/snapshot")) {
				return { projects: [], threads: [{ id: "thread-123", latestTurn: { state: "completed", assistantMessageId: "msg-9" }, messages: [{ id: "msg-9", role: "assistant", text: "Done — applied the guard clause." }] }] };
			}
			return {};
		});
		const reply = await run_crit_agent({ origin: mock.origin, token: "t", threadId: "thread-123", timeoutMs: 2000, pollMs: 50, stdin: JSON.stringify({ comment: "fix", filePath: "a.ts", startLine: 1, endLine: 1 }) });
	...
	it("returns an ack when the turn does not complete before timeout", async () => {
		mock = await start_mock_gits((url) => {
			if (url.endsWith("/api/orchestration/snapshot")) return { projects: [], threads: [{ id: "thread-123", latestTurn: { state: "running", assistantMessageId: null }, messages: [] }] };
			return {};
		});
		const reply = await run_crit_agent({ origin: mock.origin, token: "t", threadId: "thread-123", timeoutMs: 300, pollMs: 50, stdin: JSON.stringify({ comment: "fix", filePath: "a.ts", startLine: 1, endLine: 1 }) });
