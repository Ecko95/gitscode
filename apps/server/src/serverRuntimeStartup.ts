...
export const makeServerRuntimeStartup = Effect.gen(function* () {
  ...
  const startup = Effect.gen(function* () {
    ...
    yield* Effect.logDebug("startup phase: graveyard orphan adoption scan");
    yield* runStartupPhase(
      "graveyard.orphan.scan",
      graveyardOrphanAdopter.run.pipe(Effect.forkScoped),
    );
    ...
    Effect.gen(function* () {
      ...
      if (serverConfig.startupPresentation === "headless") {
        ...
        if (serverConfig.mode !== "desktop") {
          yield* Effect.logInfo("Authentication required. Open GITS using the pairing URL.").pipe(
