  ...
  describe("allocate", () => {
    it.effect("creates shim env vars and an executable shim file", () =>
      Effect.gen(function* () {
        ...
        const pathEntries = (result.vars["PATH"] ?? "").split(":");
        ...
        if (result.vars["GITS_ALLOWED_ROOT"] !== allowedRoot) {
          throw new Error(
            `Expected GITS_ALLOWED_ROOT '${allowedRoot}', got '${result.vars["GITS_ALLOWED_ROOT"]}'`,
          );
        }
    ...
    it.effect("shim script starts with #!/bin/sh and contains policy env vars", () =>
      Effect.gen(function* () {
        ...
        const shimPath = path.join(config.baseDir, "gits-shims", sessionId, "git");
