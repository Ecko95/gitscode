...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("maps user-stop turn completion into ready session state without error", async () => {
    ...
    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-stop-turn-started"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-stop"),
    });

    await waitForThread(
      harness.readModel,
      (thread) =>
        thread.session?.status === "running" && thread.session?.activeTurnId === "turn-stop",
    );

    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-completed-user-stop"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: "2026-01-01T00:00:00.000Z",
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-stop"),
      status: "interrupted",
      errorMessage: "result_type=user stop_reason=tool_use",
    });
  ...
  it("caps item.updated data at 4 KB, passes small data unchanged", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-data-turn-started"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
    ...
    harness.emit({
      type: "item.updated",
      eventId: asEventId("evt-data-small"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-data"),
      itemId: asItemId("item-data"),
      payload: {
        itemType: "command_execution",
        title: "Run",
        data: smallData,
    ...
    harness.emit({
      type: "item.updated",
      eventId: asEventId("evt-data-big"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-data"),
      itemId: asItemId("item-data2"),
      payload: {
        itemType: "command_execution",
        title: "Run big",
        data: bigData,
