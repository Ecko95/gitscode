...
function GitsWordmark() {
  return <span className="shrink-0 font-mono text-sm font-semibold tracking-normal">GITS</span>;
}
      ...
      <Tooltip>
        <TooltipTrigger
          render={
            ...
            >
              <GitsWordmark />
              <span className="rounded-full bg-muted/50 px-1.5 py-0.5 text-[8px] font-medium uppercase tracking-normal text-muted-foreground/60">
                {APP_STAGE_LABEL}
              </span>
            </Link>
          }
    ...
    <SidebarFooter className="p-2">
      ...
      <SidebarMenu>
        ...
        <SidebarMenuItem>
          <SidebarMenuButton
            size="sm"
            isActive={pathname.startsWith("/gits")}
            className="gap-2 px-2 py-1.5 text-muted-foreground/70 hover:bg-accent hover:text-foreground"
            onClick={handleGitsClick}
          >
